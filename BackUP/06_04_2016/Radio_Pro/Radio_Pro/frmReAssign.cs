        ...
        private void btnAssignData_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (mySelectDT != null && mySelectDT.Rows.Count > 0)
                {
                    ...
                    {
                        Mycmd = connect.CreateCommand();
                        Mycmd.CommandText = "select radioPatientRptDate,radioPatientsId,radioPatientsName,radioDoctorId,radioName,cut,invest,deviceId,userName,status,pt_transaction from csvfile where userName='" + deviceId + "' and radioPatientRptDate between'" + startDate + "'and'" + endDate + "' and status=0";
                        Mycmd.ExecuteNonQuery();
