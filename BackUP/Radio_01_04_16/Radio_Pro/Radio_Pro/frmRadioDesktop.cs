...
namespace Radio_Pro
{
    public partial class frmRadioDesktop : Form
    {
        ...
        private void btnRefress_Click(object sender, EventArgs e)
        {
            ...
            connect.Open();
            Mycmd = connect.CreateCommand();
            Mycmd.CommandText = "select pt_transaction,flag from servertransactions";
            Mycmd.ExecuteNonQuery();
            ...
            if (mydtTrans.Rows.Count > 0)
            {
                ...
                {
                    Mycmd.CommandText = "update csvfile set status='" + trID[1].ToString() + "' where pt_transaction='"+trID[0]+"'";
                    Mycmd.ExecuteNonQuery();
                }
            }
            connect.Close();
